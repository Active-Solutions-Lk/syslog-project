  ...
  const menuItems = [
    ...
    },
    {
      ...
      submenu: [
        ...
        { title: "Devices Settings", href: "/settings/devices" },
              ...
              {item.hasSubmenu ? (
                <div className="relative">
                  <SidebarMenuButton
                    className={cn(
                      ...
                      pathname.startsWith(item.href) && "bg-transparent text-xs font-medium"
                    )}
                    onClick={() => setSettingsOpen(!settingsOpen)}
                  >
                    <item.icon className={cn("h-3 w-3 md:h-4 md:w-4", item.color)} />
                ...
                <Link href={item.href} passHref>
                  <SidebarMenuButton
                    className={cn(
                      "w-full justify-start gap-2 hover:bg-slate-100",
                      pathname === item.href && "bg-transparent text-xs md:text-sm font-extrabold"
                    )}
                  >
                    <item.icon className={cn("h-3 w-3 md:h-4 md:w-4", item.color)} />
                    <span className="text-xs md:text-sm">{item.title}</span>
