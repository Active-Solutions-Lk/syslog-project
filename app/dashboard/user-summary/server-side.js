export async function getProcessedUserSummaries() {
  try {
    ...
    console.log('Fetched data:', { users: users.length, devices: devices.length, logs: logs.length, extensionToCategory });
    ...
    for (const user of users) {
      userMap.set(user.id, {
        ...
        fileTypes: {
          ...
          Document: { createCount: 0, readCount: 0, deleteCount: 0, totalSize: 0 },
  ...
  // ✅ Skip if no file extension found
  if (!extension) {
  ...
  const fileTypeStats = userObj.fileTypes[category] || userObj.fileTypes.Other;

  if (log.event === 'upload' || log.event === 'copy') fileTypeStats.createCount += 1;
